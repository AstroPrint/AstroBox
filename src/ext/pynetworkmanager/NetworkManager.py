...
# easier.
#
# https://github.com/seveas/python-networkmanager - commit aea55e82cfd888e12bb4cd7b7c4fffba1a6c09ba
#
# Modified by AstroPrint to allow for removal of signal handlers.
# Changes in:
#   - SignalDispatcher.remove_signal_receiver
#   - SignalDispatcher.add_signal_receiver
#   - NMDbusInterfaceType.make_signal
#   - ActiveConnection.__new__
#
...
class SignalDispatcher(object):
		...
		def add_signal_receiver(self, interface, signal, obj, func, args, kwargs):
				...
				receiver = (obj, func, args, kwargs)
				self.handlers[key].append(receiver)

				return (key, receiver)

		def remove_signal_receiver(self, listener):
				key, receiver = listener

				if key in self.handlers:
					idx = self.handlers[key].index(receiver)
					del self.handlers[key][idx]

					if len(self.handlers[key]) == 0:
...
class NMDbusInterfaceType(type):
		...
		def make_signal(klass, interface, attrib):
				...
				code += "    return SignalDispatcher.add_signal_receiver('%s', '%s', self, func, list(args), kwargs)"  % (interface, name)
				exec(code, globals(), ret)
				return ret['On' + name]
		...
		def __new__(klass, object_path):
				if klass == ActiveConnection:
						...
						try:
								...
								# In old implementation this exception happens on occasion
								if e.get_dbus_name() == 'org.freedesktop.DBus.Error.UnknownMethod':
										pass
								else:
										raise
