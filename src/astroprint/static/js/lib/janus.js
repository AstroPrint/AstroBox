...
Janus.init = function(options) {
  ...
  if(Janus.initDone === true) {
    ...
    /*function addJs(src) {
      ...
      if(src === 'adapter.js') {
        if(adapter.getUserMedia && adapter.RTCPeerConnection) {
          // Already loaded
          return;
        }
      }
...
Janus.isWebrtcSupported = function() {
  if(adapter.RTCPeerConnection === null || adapter.getUserMedia === null) {
    return false;
  }
  return true;
};
...
function Janus(gatewayCallbacks) {
  ...
  function streamsDone(handleId, jsep, media, callbacks, stream) {
    ...
    config.pc = new RTCPeerConnection(_.extend(pc_config, {rtcpMuxPolicy: "negotiate"}), pc_constraints);
    ...
    config.pc.onicecandidate = function(event) {
      if (event.candidate == null ||
          (adapter.webrtcDetectedBrowser === 'edge' && event.candidate.candidate.indexOf('endOfCandidates') > 0)) {
      ...
      if(videoSupport === true && media != undefined && media != null) {
        if(media.video && media.video != 'screen') {
          ...
          function getScreenMedia(constraints, gsmCallback) {
            ...
            adapter.getUserMedia(constraints,
              function(stream) {
  ...
  function createOffer(handleId, media, callbacks) {
    ...
    if(adapter.webrtcDetectedBrowser == "firefox" || adapter.webrtcDetectedBrowser == "edge") {
      mediaConstraints = {
  ...
  function getVolume(handleId) {
    ...
    if(config.pc.getStats && adapter.webrtcDetectedBrowser == "chrome") { // FIXME
  ...
  function getBitrate(handleId) {
    ...
    if(config.pc.getStats && adapter.webrtcDetectedBrowser == "chrome") {
    ...
    } else if(config.pc.getStats && adapter.webrtcDetectedBrowser == "firefox") {
  ...
  function isVideoSendEnabled(media) {
    ...
    if(adapter.webrtcDetectedBrowser == "edge") {
